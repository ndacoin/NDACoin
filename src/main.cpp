...
#if defined(NDEBUG)
# error "NDACOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2a61b1fbbab49f3a12e2af43ef0432c859bcf6a90afe309be3fc62cd06689917");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NDACOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NDACOIN Signed Message:\n";
...
{
    // NDACOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 12750000 * COIN;
            }

    // Subsidy is cut in half every 510000 blocks
    nSubsidy >>= (nHeight / 510000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 8 * 60;
static const int64 nTargetSpacing = 8 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NDACOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NDACOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1525181736, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "This time is right to launch NDACOIN to help humanity by RS";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0422c6da00cc9aebf762fb27be3b0bb6632fe04e02ad2a72f532d89388d83a0e88e2a612d7d114ac632048e0b0375a39c599ae90883fb56990feec7a408587ba60") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525181736;
        ...
        assert(block.hashMerkleRoot == uint256("0x65dfcdb9b4c4d9cc09ae52d54b25ed23b51bbe250ace83209b8d4a218231854c"));
...
unsigned char pchMessageStart[4] = { 0xb6, 0x30, 0x3b, 0xfb }; // NDACOIN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NDACOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NDACOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NDACOINMiner(CWallet *pwallet)
{
    printf("NDACOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ndacoin-miner");
    ...
    try { loop {
        ...
        printf("Running NDACOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NDACOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NDACOINMiner, pwallet));
}
